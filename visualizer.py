...
# Mesh Rendering
def render_mesh(mesh, output_path, num_views=120, image_size=512, distance=2.7, elevation=30, textures=None, fov=60, fps=30, elevat=1):
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # mesh = mesh.to(device)
    ...
    R, T = pytorch3d.renderer.look_at_view_transform(dist = distance, elev = elevat, 
                                                     azim =azimuth)
